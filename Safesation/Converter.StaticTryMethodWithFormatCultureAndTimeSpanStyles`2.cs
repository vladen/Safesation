...
namespace Safesation
{
    ...
    {
        private sealed class StaticTryMethodWithFormatCultureAndTimeSpanStyles<TSource, TTarget>
            : Converter<TSource, TTarget>
        {
            #region CONSTRUCTORS

            public StaticTryMethodWithFormatCultureAndTimeSpanStyles(MethodInfo method)
                : base(ConversionFeatures.CultureFormatStyles, ConversionWays.StaticTryMethod)
            {
            ...
            protected override Delegate Build()
            {
                ...
                il.Emit(OpCodes.Ldarga_S, ConversionArgument);
                il.Emit(OpCodes.Call, GetTimeSpanStyles);
                il.Emit(OpCodes.Ldloca_S, ConversionArgument);
