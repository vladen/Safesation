...
namespace Safesation
{
    ...
    {
        private sealed class InstanceMethodWithFormat<TSource, TTarget>
            : Converter<TSource, TTarget>
        {
            #region CONSTRUCTORS

            public InstanceMethodWithFormat(MethodInfo method)
                : base(ConversionFeatures.Format, ConversionWays.InstanceMethod)
            {
