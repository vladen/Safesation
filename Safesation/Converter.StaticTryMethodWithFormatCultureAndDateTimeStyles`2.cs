...
namespace Safesation
{
    ...
    {
        private sealed class StaticTryMethodWithFormatCultureAndDateTimeStyles<TSource, TTarget>
            : Converter<TSource, TTarget>
        {
            #region CONSTRUCTORS

            public StaticTryMethodWithFormatCultureAndDateTimeStyles(MethodInfo method)
            ...
            protected override Delegate Build()
            {
                ...
                il.Emit(OpCodes.Ldarga_S, ConversionArgument);
                il.Emit(OpCodes.Call, GetDateTimeStyles);
